        ...
        for index in range(start, start+count):
            for n, task in enumerate(tasks):
                ...
                if clim_set == (0,0):
                    ...
                    paxes, caxes = plot_tools.plot_bot_3d(dset, 0, index, axes=axes, title=task, even_scale=True, visible_axes=False, func=func, cmap=cmap,clim=clim_set)
                
                paxes.axis('off')
                # caxes.cla()
                # caxes.axis('off')
                cax_pos = caxes.get_position()
                cax_pos.y0 += 0.03  # shift bottom up
                cax_pos.y1 += 0.03  # shift top up
                caxes.set_position(cax_pos)
