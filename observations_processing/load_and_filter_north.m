...
load PJ4_north_r4_1961_1961_lon_lat_vx_vy_vor_v0.mat
...
hold on
dat = vorticity/f;
m_proj('stereographic','lat',90,'long',0,'radius',10);
...
exportgraphics(gcf,'plots/rawdata_N.png','resolution',500)
...
lam = deg2rad(lon);  % longitude [rad]
theta = pi/2 - phi;      % colatitude [rad]
rho   = R * theta;       % physical distance from pole [m]
x_grid = rho .* sin(lam);   % +x towards 90E
y_grid = -rho .* cos(lam);  % +y towards 180W


% Put Velocities in NS/EW Coords
u_east =  u .* cosd(alpha) + v .* sind(alpha);
v_north = -u .* sind(alpha) + v .* cosd(alpha);

%% trim away NaN parts
vorticity(1:2,1)=NaN;
nanmask = ~isfinite(vorticity);
x_grid(nanmask) = NaN;
y_grid(nanmask) = NaN;

% xmin = min(x_grid,[],"all","omitnan");
% xmax = max(x_grid,[],"all","omitnan");
% ymin = min(y_grid,[],"all","omitnan");
% ymax = max(y_grid,[],"all","omitnan");
...
gridmask =  xmin <= x_grid & x_grid <= xmax & ymin <= y_grid & y_grid <=ymax;
...
dx = 50e3; % reasonable compared to uncertainty
dy = dx;
...
exportgraphics(gcf,'plots/interpolated_raw_N.png','resolution',500)
...
exportgraphics(gcf,'plots/filtered_N.png','resolution',500)
